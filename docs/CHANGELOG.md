...
- refactor(sse): SSE 协议词汇找属主（架构评审第2轮候选3）——新增 `utils/sse_protocol.py` 单一词汇表（11 token 常量 + `frame` 帧构造 + `parse_frame` 帧解析，双帧形语义：裸式 payload 与包裹式尾括号终结符）；发射方 `api/chat_stream.py`/`agent/react_agent.py`（inband_thinking）与消费方 `scripts/benchmark.py` 全部换用，消灭魔法切片；app.js 顶部抽 `SSE_PROTOCOL` 词表 + `parseSSEFrame` 统一解析器，processLine 按裸 token 分派、历史消息 `_extractCharts` 正则从词表构建，双解析器共用同一词汇源；**顺修 [TRACE] 潜伏漏洞**（OTel 激活时 hex 穿透所有分支渲染进气泡，现前端显式忽略）；新增跨语言锁步契约测试（读 app.js 提取词表与 Python TOKENS 双向比对）。线上字节级不变
