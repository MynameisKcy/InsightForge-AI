...
| `config/agent.yml` | 外部数据路径 + DuckDB 查询通道资源上限 | `external_data_path: data/external/records.csv`；`duckdb:` 节 `memory_limit: 1GB` / `threads: 2` / `max_result_rows: 10000` / `query_timeout_seconds: 30`（经连接配置应用，SET/PRAGMA 仍被沙箱拒绝） |
