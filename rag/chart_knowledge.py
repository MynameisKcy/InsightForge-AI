...
class ChartKnowledgeBase:
    ...
    def save_chart(self, chart_info: dict) -> int:
        """保存图表信息到知识库。
        chart_info = {
            "chart_type": "line",
            "title": "月度销售趋势",
        ...
        with self._get_conn() as conn:
            cursor = conn.execute(
                """INSERT INTO chart_archive
                   (chart_type, title, x_col, y_col, data_summary, analysis_text,
                    chart_path, task_context, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
            ...
            logger.info(f"ChartKnowledge: saved chart #{chart_id} - {chart_info.get('title')}")
            return chart_id
    ...
    def search_by_type(self, chart_type: str, limit: int = 5) -> list[dict]:
        """按图表类型检索历史图表。"""
        with self._get_conn() as conn:
            rows = conn.execute(
                """SELECT * FROM chart_archive WHERE chart_type = ?
                   ORDER BY created_at DESC LIMIT ?""",
                (chart_type, limit),
            ).fetchall()
            return [dict(r) for r in rows]

    def get_recent_charts(self, limit: int = 10) -> list[dict]:
        """获取最近的图表记录。"""
        with self._get_conn() as conn:
            rows = conn.execute(
                "SELECT * FROM chart_archive ORDER BY created_at DESC LIMIT ?",
                (limit,),
            ).fetchall()
            return [dict(r) for r in rows]

    def get_chart_context_for_rag(self, query: str, max_charts: int = 5) -> str:
        """为 RAG 检索提供历史图表上下文。
        根据用户查询找到相关历史图表，返回格式化的参考文本。
        """
        charts = self.search_by_keywords(query, limit=max_charts)
        if not charts:
            charts = self.get_recent_charts(limit=max_charts)
    ...
    def build_insight_prompt(self, query: str, chart_specs: list[dict]) -> str:
        """生成分析建议的提示词，结合历史图表数据和外部搜索建议。
        用于指导 LLM 生成更深入的分析建议。
        """
        # 获取相关历史图表
        chart_context = self.get_chart_context_for_rag(query, max_charts=5)
...
"""
    ...
    def clear_old_data(self, days: int = 90):
        ...
        with self._get_conn() as conn:
            conn.execute("DELETE FROM chart_archive WHERE created_at < ?", (cutoff,))
            conn.commit()
