...
The SSE wire contract of `/api/chat` and its single implementation, `api/chat_stream.py`: preamble tokens (`[SESSION]`/`[TRACE]`/`[SESSIONS_RELOAD]`), progress-event routing (`[METRICS]`/`[DECISION]`/`[STEP]`), `[THINKING]` passthrough, sentence-paced body frames, disconnect sampling (mandatory at heartbeats + every 20 chunks), chart diff emission (`[CHART:url]`), and the cancellation contract (no `[DONE]`, no charts, no memory write). The route only parses the request and assembles `StreamingResponse`; the thread/heartbeat bridge lives in `api/sse.py`.
_Avoid_: "SSE 处理逻辑散在路由里" (retired — the route is thin); putting the wire vocabulary in more than one module.
