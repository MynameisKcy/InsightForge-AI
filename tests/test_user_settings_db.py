...
def test_upsert_get_roundtrip(tmp_path):
    ...
    db.upsert("u1", {"llm_api_key": "test-api-key-caa0410b364d47f09774d0c3b2b64213",
                     "llm_model_name": "qwen-max",
                     "embedding_model_name": "text-embedding-v2",
                     "local_db_conn": "sqlite:///db.db"})
    got = db.get("u1")
    assert got["llm_api_key"] == "test-api-key-caa0410b364d47f09774d0c3b2b64213"
...
def test_masked_hides_full_key(tmp_path):
    ...
    assert masked["llm_api_key"].startswith("test-api-key-")
...
def test_storage_is_encrypted(tmp_path):
    db = _fresh_db(tmp_path)
    db.upsert("u1", {"llm_api_key": "test-api-key-secretkey123456",
                     "llm_model_name": "qwen-max"})
    ...
    conn.close()
    assert "test-api-key-secretkey123456" not in row[0]
